class RankManager:
    ...
    def _create_rank_zoos(self):
        ...
        world_size = self.args.fake_pp * self.args.fake_tp * self.args.fake_dp
...
class RankZoo:
    def __init__(self, world_rank, local_rank, server_id, model_config=None,
                 device_type='gpu', dp_groups=None, pp_groups=None, tp_groups=None, mp_groups=None, ep_groups=None, pep_groups=None):
    ...
    def _get_ep_group_size(self):
        return len(self.ep_groups)
    
    def _get_pep_group_size(self):
        return len(self.pep_groups)
    ...
    def _get_tp_local_rank(self):
        """ Get the stage id in the tp group"""
    ...
    def is_rank_in_embedding_group(self):
        if self.ep_groups is None:
            return False
        elif self.world_rank in self.ep_groups:
            return True
        else:
            return False
