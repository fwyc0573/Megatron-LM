...
from megatron.profiler.v1 import profiler, comm_hook, context_tracker
from megatron.profiler.v1.profiler_manager import ProfilerManager
from megatron.profiler.v1.communication_hooks import CommunicationHook
...
def pretrain(train_valid_test_dataset_provider,
    ...
    if args.is_scaling_mode:
        ...
        # 替换CMD.set_current_profile_sign为v1模块初始化
        rank_id_for_profiler = str(current_fake_rank_id_int) if args.is_scaling_mode else str(torch.distributed.get_rank())
        stage_id_for_profiler = args.pp_rank if args.is_scaling_mode else parallel_state.get_pipeline_model_parallel_rank()
        
        profiler.initialize(
            rank_id=rank_id_for_profiler,
            stage_id=stage_id_for_profiler,
            enabled=True
        )
        
        # 安装通信hooks（仅在scaling模式下需要手动控制）
        if args.is_scaling_mode:
            comm_hook.install()
        ...
        # 设置当前pipeline阶段
        context_tracker.set_stage(context_tracker.PipelineStage.STEADY)
        context_tracker.set_iteration(args.trace_start)
        
        with profiler.trace_operation(
            op_name="backward_step",
            stage="steady",
            group_kind="compute",
            description="backward computation step"
        ):
        ...
        with profiler.trace_operation(
            ...
            stage="steady", 
            group_kind="dp",
            tensor_shape=[args.global_model_params_dict[pos_p_t]["elem_sum"]],
            tensor_dtype=used_dtype,
            description="data parallel allreduce"
        ):
            # 这里是模拟操作，不需要实际执行
            pass

        # ep_allreduce - 替换CMD记录
        if (args.is_rank_in_embedding_group and args.fake_pp > 1):
            ...
            with profiler.trace_operation(
                op_name="ep_allreduce",
                stage="steady",
                group_kind="ep", 
                tensor_shape=list(ep_input__shape) if ep_input__shape is not None else None,
                tensor_dtype=ep_input__dtype,
                description="expert parallel allreduce"
            ):
                # 这里是模拟操作，不需要实际执行
                pass

        # optimizer.step() - 替换CMD上下文管理器
        with profiler.trace_operation(
            ...
            total_param_count = sum(param.numel() for param in params)
            grads_for_norm = optimizer.get_main_grads_for_grad_norm()
            total_grad_count = sum(grad.numel() for grad in grads_for_norm)
        ...
        profiler.export_traces(output_path="profiler_log", name_prefix=name_args)
    ...
    if not args.skip_train:
        ...
        if args.do_train and args.train_iters > 0:
            # Initialize all profiling components - 更新v1模块初始化方式
            rank_id_str = str(torch.distributed.get_rank()) if not args.is_scaling_mode else str(args.fake_current_rank_id)
            stage_id_val = parallel_state.get_pipeline_model_parallel_rank() if not args.is_scaling_mode else args.pp_rank
...
def train_step(forward_step_func, data_iterator,
    ...
    with profiler.trace_operation(
        ...
        stage="steady",
        group_kind="compute",
        description="optimizer parameter update in training"
    ):
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    # 将cmd写入txt - 更新为v1模块导出方式
    if args.do_trace and hasattr(args, 'stage_operations_trace') and args.simu_rank in args.stage_operations_trace:
        # 使用传统方式保持兼容性
        ...
        from megatron.profiler.cmd import write_list_to_file
