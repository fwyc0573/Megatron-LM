...
def compute_weight_and_optimizer_memory(args, verbose=False, dp_size=1, pp_size=1, tp_size=1, expert_num=None):
    ...
    # MoE.
    num_experts = 1 if expert_num is None else expert_num
    ...
    num_parameters_on_most_loaded_model_shard = (
        (num_parameters_in_transformer_layers / pp_size) + embedding_size
    ) / tp_size
    if args.untie_embeddings_and_output_weights and pp_size == 1:
        num_parameters_on_most_loaded_model_shard += (
            embedding_size / tp_size
        )
    # if verbose:
    #     print(
    ...
    num_bytes_per_parameter = (
        18 if not args.use_distributed_optimizer else 6 + (12 / dp_size)
    )
...
def compute_activation_memory(args, num_microbatches, verbose=False, dp_size=1, pp_size=1, tp_size=1):
    ...
    if verbose:
        print(
            ...
            f"{activation_memory / NUM_BYTES_IN_MEGABYTE / tp_size:.1f} MB"
        )
    ...
    activation_memory += (
        8 * args.seq_length * args.micro_batch_size * pp_size
    )
    ...
    if args.virtual_pipeline_model_parallel_size is not None:
        interleaved_schedule_memory_penalty = 1 + (
            (args.pp_size - 1)
            / (args.pp_size * args.virtual_pipeline_model_parallel_size)
        )
        in_flight_microbatches = math.ceil(
            interleaved_schedule_memory_penalty * args.pp_size
        )
        if verbose:
            print(
    ...
    if args.virtual_pipeline_model_parallel_size is None and pp_size > 1:
        if num_microbatches is not None:
            activation_memory *= min(1, num_microbatches / pp_size)
            in_flight_microbatches = min(num_microbatches, pp_size)
        else:
            in_flight_microbatches = pp_size
        if verbose:
            print(f"Number of in-flight microbatches: {in_flight_microbatches}")
    ...
    return activation_memory / tp_size


def report_theoretical_memory(args, num_microbatches=None, verbose=False):
    if args.is_scaling_mode:
        dp_size = args.fake_dp
        pp_size = args.fake_pp
        tp_size = args.fake_tp
        expert_num = getattr(args, "fake_num_experts", None)
    else:
        dp_size = args.data_parallel_size
        pp_size = args.pipeline_model_parallel_size
        tp_size = args.tensor_model_parallel_size
        expert_num = getattr(args, "num_experts", None)


    weight_and_optimizer_memory = (
        compute_weight_and_optimizer_memory(args, verbose=verbose, dp_size=dp_size, pp_size=pp_size, tp_size=tp_size, expert_num=expert_num) / NUM_BYTES_IN_MEGABYTE
    )
    ...
    if not args.sequence_parallel or args.recompute_granularity != 'selective':
        print(
            f"Theoretical memory footprints (estimated): weight and optimizer={weight_and_optimizer_memory:.2f} MB"
        )
        # return

        # Here we change the logic to get total_memory.
        activation_memory = (
            compute_activation_memory(args, num_microbatches=num_microbatches, verbose=verbose, dp_size=dp_size, pp_size=pp_size, tp_size=tp_size)
