...
class _LayerNormLinear(torch.autograd.Function):
    ...
    def forward(
        ...
        nvtx.range_push(f"normlinear_fwd_{parallel_mode}_LayerNormLinear")
        ...
        if parallel_mode == "row" and sequence_parallel:
            ...
            nvtx.range_push(f"normlinear_fwd_row_allreduce")
            out, _ = allreduce(out, tp_group, func='normlinear_fwd')
            nvtx.range_pop()
    ...
    def backward(
        ...
        nvtx.range_push(f"norwlinear_bwd_{ctx.parallel_mode}_LayerNormLinear")
        with _prepare_backward(
            ...
            if ctx.parallel_mode == "column" and ctx.sequence_parallel:
                ...
                nvtx.range_push(f"normlinear_bwd_column_allreduce")
                CMD.async_start_trace("normlinear_bwd_async", {'input_': dgrad, 'func': 'normlinear_bwd'}, {'input_': ['shape', 'dtype'], 'func': ['name']}, group_type='tp', comm_func='allreduce')
                dgrad, handle = allreduce(dgrad, ctx.tp_group, async_op=True, func='normlinear_bwd')
                raise 0
            ...
            if ((not ctx.ub_bulk_wgrad)
                ...
                handle.wait()
                CMD.async_end_trace("normlinear_bwd_async")
                raise 0
                nvtx.range_pop()
...
class LayerNormLinear(TransformerEngineBaseModule):
    ...
    def __init__(
        ...
        #         # print("tp_group is None, tp_size == 1")
        #         # raise 0
        #     else:
        #         self.tp_size = get_distributed_world_size(tp_group)
        #         self.set_tensor_parallel_group(tp_group)
        #         # print(f"tp_group is not None, tp_size == {tp_size}")
        #         # raise 0
        # else:
        #     self.tp_size = fake_tp
        #     self.set_tensor_parallel_group(tp_group)
        #     tp_size = get_distributed_world_size(tp_group)
        #     # print(f"LayerNormLinear | fake mode | tp_group is not None, fake_tp == {fake_tp}, real_tp == {tp_size}")
        #     # raise 0

        
        # Original logic for tp_group and tp_size
        if tp_group is None:
            self.tp_size = tp_size
            # If tp_size is > 1 and tp_group is None,
            # TE might expect set_tensor_parallel_group to be called later.
            # For a single GPU simulation where tp_group is None, this is fine.
            if tp_size == 1: # Or other conditions based on original TE logic
                 self.set_tensor_parallel_group(tp_group)
            # else: # Potentially handle cases where tp_size > 1 and tp_group is None
                 # For simulation, this might mean self.tp_group remains None.
        else:
            self.tp_size = get_distributed_world_size(tp_group)
            self.set_tensor_parallel_group(tp_group)
        ...
        if parameters_split is None:
            ...
            # print("Default split:", self.weight_names, self.bias_names, self.parameter_split_sizes)
            ...
            print("Dictionary split:", self.weight_names, self.bias_names, self.parameter_split_sizes)
            ...
            for name in parameters_split:
                ...
                self.bias_names.append(f"{name.rstrip('_')}_bias")
                self.parameter_split_sizes.append(split_size)
            print("Even split:", self.weight_names, self.bias_names, self.parameter_split_sizes)
        else:
