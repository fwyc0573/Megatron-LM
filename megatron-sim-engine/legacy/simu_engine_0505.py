"""
该version未将add opearion的主逻辑写到timelinemanager中
已完成和data control group模块的连接
已添加了一些必要的依赖和匹配关系
...
from rank_manager import RankZoo
...
class IndividualTimeline:
    ...
    def _add_operation_to_timeline(self, operation:Operation, global_finished_operations:dict, \
        ...
        can_go_on_sign = True
        next_operation = None
        print(f"dependency_relationship: {dependency_relationship}, operation.name = {operation.name}")
    ...
    def _get_dependency_operation_name_and_offset(self, operation: Operation, dependency_relationship: dict):
        tmp_value = dependency_relationship[self.stage_kind][operation.name]
    ...
    def _operation_dependency_finished(self, global_finished_operations: dict, dependency_relationship: dict, operation: Operation) -> Union[bool, Operation]:
        dependencies = self._get_dependency_operation_name_and_offset(operation, dependency_relationship)
        ...
        for dependency in dependencies:
            ...
            if target_comb_operation_name in global_finished_operations:
                current_operation = global_finished_operations[target_comb_operation_name]
...
class SimulatorEngine():
    ...
    def __init__(self, stages_num=None, stages_steps_dict=None, grad_acc=None, tmp_filename=None, framwork='megatron-lm', strategy="1F1B-none_interleaved"):
    ...
    def _init_tmp_stages_dataset_and_timeline_manager(self, rank_instances_dict):
        ...
        self.timeline_manager = TimelinesManager(dependency_relationship=self.dependency_relationship, comm_matching_relationship=self.comm_matching_relationship, stages_list = self.stages_list, strategy=self.strategy)
    ...
    def _get_dependency_relationship(self, framwork=None, strategy=None):
        ...
        if framwork == "deepspeed":
            ...
            if strategy == "1F1B-none_interleaved" and self.mpu.tp_size == 1 and self.mpu.dp_size == 1:
                ...
                # 2D pp,tp并行: pp>1,tp>1,dp=1: 没有dp的allreduce，涉及pp的p2p和tp的allreduce\broadcast
                ...
                return {"FirstStage": {'forward_step': ['load_batch_broadcast', 0], 'send_forward': ['forward_step', 0], 'backward_step': ['recv_backward', 0]}, 
                        "MiddleStage": {'forward_step': ['recv_forward', 0], 'send_forward': ['forward_step', 0], 'backward_step': ['recv_backward', 0], 'send_backward': ['backward_step', 0]},
                        "LastStage": {'forward_step': [['recv_forward', 0],['load_batch_broadcast', 0]], 'send_backward': ['backward_step', 0]}}
    ...
    def _get_comm_matching_relationship(self, framwork=None, strategy=None):
        ...
        if framwork == "deepspeed":
            ...
            if self.mpu.tp_size > 1:
                # load_batch_broadcast 暂时没用，根据逻辑区分即可
                return_dict = {**return_dict, **{
                    'load_batch_broadcast': ['load_batch_broadcast', ['tp_src_rank', 'tp_group']],
                    'all_reduce_tp': ['all_reduce_tp', 'tp_group']
                }}

            if self.mpu.dp_size > 1:
                return_dict = {**return_dict, **{
                    'all_reduce_dp': ['all_reduce_dp', 'dp_group']
                }}
            return return_dict
        else:
            raise ValueError(f"The framework '{framwork}' is not supported.")
...
if __name__ == '__main__':
    ...
    filename = r"H:\HUBOther\ML_Sys_Merak\TorchGraph\megatron_operation_log\4pp_1_1"
