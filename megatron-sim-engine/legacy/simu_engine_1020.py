...
class TimelinesManager:
    ...
    def get_op_json_db_record(self):
        import json
        record_json_path = r"H:\HUBOther\ML_Sys_Merak\TorchGraph\megatron_operation_log\analysis_log\trace_dict.json"
        
        # 确保文件夹存在
        os.makedirs(os.path.dirname(record_json_path), exist_ok=True)

        # 初始化空的 trace_data 用于写入
        trace_data = {}
        ...
        for key, value in self.trace_stages_dict.items():
            ...
            for i in range(len(self.stages_timeline_process_dict[key].operations_list)):
                current_op = self.stages_timeline_process_dict[key].operations_list[i]
                # 将操作转换为字典
                op_dict = current_op.to_dict()
                op_dict["GPU_type"] = "NVIDIA_H800"
                op_dict["inter_network_type"] = "IB"
                op_dict["intra_network_type"] = "NVLINK"
                # 将操作记录添加到该 rank 下
        ...
        #             current_op = self.trace_stages_dict[key].operations_list[i]


        #             if isinstance(current_op, SubOperation):
        #                 print(f"type:{type(current_op)}, current_op={current_op}")
...
class SimulatorEngine():
    ...
    def get_op_json_db_record(self):
        if self.running_mode == MODE_SIMULATE:
            self.timeline_manager.get_op_json_db_record()
