...
class SchedulingPlan:
    def __init__(self, args):
        ...
        if args.fp16:
            ...
            self.args.pipeline_dtype = torch.bfloat16
    ...
    def write_list_to_file(self, stage_id, list_to_write):
        ...
        explicit_output_dir = getattr(self.args, "output_dir", None)
        if explicit_output_dir is not None:
            log_dir = os.path.abspath(explicit_output_dir)
        else:
            dir_name = f"MODEL{self.args.model_size}_pp{self.args.pipeline_model_parallel_size}_tp{self.args.tensor_model_parallel_size}_dp{self.args.data_parallel_size}_exp{self.args.expert_model_parallel_size}_seq{self.args.seq_length}_mbs{self.args.micro_batch_size}_gbs{self.args.global_batch_size}_{precision_str}"
            ...
            log_dir = os.path.join(
                ...
                "simulation_inputs",
                "scheduling_plans",
                "mg_scheduling_plan_log",
                dir_name,
            )
        os.makedirs(log_dir, exist_ok=True)

        if explicit_output_dir is not None:
            filename = os.path.join(log_dir, f"stage{stage_id}_scheduling_plan.txt")
        else:
            # 获取当前时间信息
            current_time = datetime.now().strftime("%Y%m%d_%H%M%S")

            # 创建包含stage id和时间信息的文件名
            filename = os.path.join(log_dir, f"stage{stage_id}_{current_time}_scheduling_plan.txt")
