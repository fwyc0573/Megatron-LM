...
class RankManager:
    def __init__(self, mpu_info: MPUInfo, gpus_per_node: int, all_groups: dict):
        """Initialize the rank manager.
        
        Args:
            mpu_info: MPUInfo object with all parallel group information
            gpus_per_node: Number of GPUs per node
            all_groups: Dictionary containing all parallel groups
        """
        self.mpu_info = mpu_info
        self.gpus_per_node = gpus_per_node
        self.groups_dict = all_groups
    ...
    def _create_rank_zoos(self):
        ...
        """
        rank_zoos = {}
        world_size = self.mpu_info.world_size
        
        for world_rank in range(world_size):
            local_rank = world_rank % self.gpus_per_node
            server_id = world_rank // self.gpus_per_node
