...
MG_COMM_OPERATION = ['recv_forward', 'send_forward', 'recv_backward', 'send_backward', 'tp_allreduce', 'tp_load_batch_broadcast', 'dp_allreduce', 'ep_allreduce', 'exp_all_to_all', 'exp_allgather']
...
MOE_COMM_COLLECTIVE = ['exp_all_to_all', 'exp_allgather']
...
class TimelinesManager:
    ...
    def start_pop_operators(self):
        ...
        if self.running_mode == MODE_PROFILE:
            # PROFILE模式：直接按顺序播放每个rank的trace，不考虑跨rank依赖
            for wrank_id in sorted(self.stages_timeline_process_dict.keys()):
                ...
                for operation in timeline.waiting_queue:
                    # 在Profile模式下，所有操作（无论是comp还是comm）都按顺序执行
                    # duration已经包含了实际的等待时间，所以直接设置即可
    ...
    def _add_operation_to_timeline(self, timeline:IndividualTimeline, operation:Operation):
        ...
        if operation.op_kind == "comp":
            ...
            elif parallel_kind == "dp" or parallel_kind == "ep" or parallel_kind == "tp" or parallel_kind == "exp":
                # TODO：修正下这个写法，不太美观
                if comm_kind == "allreduce" or comm_kind == "all_to_all" or comm_kind == "allgather": # if "allreduce" in operation.name:
    ...
    def _update_matching_comm_ops_properties(self, comm_matching_operation_list: list, comm_matching_operation_format_name_list: list, 
        ...
        if comm_kind == "allreduce" or comm_kind == "all_to_all" or comm_kind == "allgather":
        ...
        elif comm_kind == "allreduce" or comm_kind == "all_to_all" or comm_kind == "allgather":
    ...
    def _get_comm_matching_operation_name_and_wrank_id(self, operation: Operation, comm_kind: str, parallel_kind: str, timeline: IndividualTimeline):
        ...
        if (comm_kind == "p2p" or comm_kind == "p2p_fused") and parallel_kind == "pp":
            ...
            if comm_kind == "all_to_all" or comm_kind == "allgather":
                return self.comm_matching_relationship[operation.name][0], [rank_id for rank_id in
                                                                            timeline.stage_rank.exp_groups if rank_id != timeline.wrank_id], None
        else:
    ...
    def _get_comm_operation_kind_and_parallel_dimension(self, operation: Operation):
        ...
        elif "all_to_all" in operation.name:
            return "all_to_all", operation.group_kind
        elif "allgather" in operation.name:
            return "allgather", operation.group_kind
        else:
    ...
    def visualize_timelines(self, running_mode, mpu, wrank_id_start_end=[1,100], specific_ranks_list=None, show_x_lim=None, 
        ...
        if save_plot:
            ...
            filename = f"timeline_{running_mode}_PP{mpu.pp_size}_TP{mpu.tp_size}_DP{mpu.dp_size}_{rank_range}_{timestamp}.png"
                ...
                for line in lines:
                    ...
                    if line:
                        ...
                        if cmd_name == "dp_allreduce" or cmd_name == "ep_allreduce":
                            ...
                            # 检查是否为无效的 ep_allreduce 操作（MOE 模型中 share_embeddings_and_output_weights=False 的情况）
                            if cmd_name == "ep_allreduce" and (tensor_shape is None or tenosr_dtype is None):
                                print(f"Skipping invalid ep_allreduce operation: tensor_shape={tensor_shape}, tenosr_dtype={tenosr_dtype}")
                                print(f"This typically occurs in MOE models with untied embeddings (share_embeddings_and_output_weights=False)")
                                # 设置 duration 为 0，因为没有实际的通信发生
                                duration = 0.0
                            else:
                                # 正常处理有效的 allreduce 操作
                                if group_kind == "dp":
                            ...
                            print(f"cmd_name:{cmd_name}, tensor_shape:{tensor_shape}, tenosr_dtype:{tenosr_dtype}, comm_group:{comm_group if 'comm_group' in locals() else 'N/A'}, duration:{duration}")
...
def get_tensor_data_size(tensor_shape:list,tensor_dtype:str)->int:
    # Handle None values for tensor_shape and tensor_dtype
