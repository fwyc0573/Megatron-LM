...
import mg_scheduling.parallel_state
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    # rank代表的当前dist.rank所在global pipeline rank中的序号（从0开始）
    ...
    ''' -------------------------------------------cooldown state-----------------------------------------'''
    import os
    def write_list_to_file(world_rank, list_to_write):
        os.makedirs("./mg_scheduling", exist_ok=True)
        filename = f"./mg_scheduling/{world_rank}_stage_operations.txt"
        with open(filename, 'w') as f:
            for item in list_to_write:
                f.write(f"{item}\n")
                
    print(f"world_rank: {args.simu_rank}, total operations num: {len(args.simu_stage_operations_trace)}")
    write_list_to_file(args.simu_rank, args.simu_stage_operations_trace)
