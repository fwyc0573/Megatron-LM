...
RUN_TAG="dryrun_$(date +%Y%m%d_%H%M%S)_$$"
RUN_DIR="${ARTIFACT_ROOT}/${RUN_TAG}"
mkdir -p "${RUN_DIR}"

OUTPUT_CSV="${RUN_DIR}/gpt175b_scaling_wallclock_timing.csv"
...
expected_header="world_size,pp_size,tp_size,dp_size,measured_ranks_count,single_iter_wallclock_seconds,estimated_15_iters_seconds"
...
mapfile -t measured_counts < <(tail -n +2 "${OUTPUT_CSV}" | cut -d"," -f5)
expected_counts=(16 16 32 32)
...
if ! awk -F"," "NR > 1 { if (NF != 7) { exit 1 } }" "${OUTPUT_CSV}"; then
  echo "[FAIL] CSV row has invalid field count (expect 7)." >&2
  exit 1
fi

if ! awk -F"," "NR > 1 { expected=sprintf(\"%.6f\", \$6 * 15); if (\$7 != expected) { exit 1 } }" "${OUTPUT_CSV}"; then
  echo "[FAIL] estimated_15_iters_seconds != single_iter_wallclock_seconds * 15." >&2
  exit 1
fi

echo "[PASS] Dry-run integration checks passed."
