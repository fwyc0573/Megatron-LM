...
# DeepSeek-V3 MoE training/profiling script.
#
# Log/trace parameter defaults (TRACE_SUBOP_SYNC_MODE, TRAIN_ITERS, etc.) are
# aligned with pretrain_qwen3_30b_a3b_moe.sh for consistent cross-model
# comparison.  In particular, scaling_replay_cache-related parameters
# (--scaling-replay-cache-tag, --scaling-min-warmup-iters,
# --scaling-profile-iters, --scaling-strict-grad-replay,
# --scaling-replay-write-phase) are NOT passed; the training code falls back
# to argument defaults / random-grad fallback.
#
# Usage:
#   MODE=distributed bash examples/pretrain_deepseek_v3_moe_aligned.sh
#   MODE=scaling     bash examples/pretrain_deepseek_v3_moe_aligned.sh
...
# ---------------------------------------------------------------------------
# Trace / logging defaults — aligned with pretrain_qwen3_30b_a3b_moe.sh
# ---------------------------------------------------------------------------
MODE=${MODE:-distributed}                   # distributed | scaling
MODEL_PROFILE=${MODEL_PROFILE:-smoke}       # smoke | full
TRANSFORMER_IMPL=${TRANSFORMER_IMPL:-local} # local required for MLA
TRACE_START=${TRACE_START:-1}
TRAIN_ITERS=${TRAIN_ITERS:-10}              # aligned with Qwen3 (was 3)
TRACE_SUBOP_SYNC_MODE=${TRACE_SUBOP_SYNC_MODE:-event}  # aligned with Qwen3 (was global)
...
# ---------------------------------------------------------------------------
# Model architecture (DeepSeek-V3 MoE with MLA)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Fake-parallel topology (scaling mode only)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Trace arguments — aligned with Qwen3 (no --trace-cmd-sync-mode)
# ---------------------------------------------------------------------------
TRACE_ARGS=(
  --do-trace "${DO_TRACE}"
...
# ---------------------------------------------------------------------------
# Common model / training arguments
# ---------------------------------------------------------------------------
COMMON_ARGS=(
  --use-mcore-models
