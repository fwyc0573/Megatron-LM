...
SCALING_TRACE_METADATA_COMM_DURATION=${SCALING_TRACE_METADATA_COMM_DURATION:-0}
...
if [[ "${MODEL_PROFILE}" == "full" ]]; then
  ...
  MAX_POSITION_EMBEDDINGS=163840 # 16384
...
if [[ "${SCALING_TRACE_METADATA_COMM_DURATION}" != "0" && "${SCALING_TRACE_METADATA_COMM_DURATION}" != "1" ]]; then
  echo "[ERROR] SCALING_TRACE_METADATA_COMM_DURATION must be 0 or 1, got ${SCALING_TRACE_METADATA_COMM_DURATION}."
  exit 1
fi
...
if [[ "${SCALING_TRACE_METADATA_COMM_DURATION}" == "1" ]]; then
  ADVANCED_FLAGS+=("SCALING_TRACE_METADATA_COMM_DURATION=1")
fi
...
if [[ "${MODE}" == "distributed" ]]; then
  ...
  SCALING_METADATA_COMM_ARGS=()
  if [[ "${SCALING_TRACE_METADATA_COMM_DURATION}" == "1" ]]; then
    SCALING_METADATA_COMM_ARGS+=(--scaling-trace-metadata-comm-duration)
  fi
