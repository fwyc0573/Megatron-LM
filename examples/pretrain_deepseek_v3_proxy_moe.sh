...
MODEL_PROFILE=${MODEL_PROFILE:-smoke} # smoke | proxy
...
if [[ "${MODEL_PROFILE}" == "proxy" ]]; then
  # DeepSeek-V3-Proxy mainline (MLA simplified to MHA in stage-1).
  NUM_LAYERS=14
  HIDDEN_SIZE=7168
  NUM_HEADS=56
  ...
  # Smoke profile for local tracing correctness.
  NUM_LAYERS=14
  HIDDEN_SIZE=2048
  NUM_HEADS=16
...
MOE_LAYER_FREQ=${MOE_LAYER_FREQ:-'([0]*3+[1]*11)'}
